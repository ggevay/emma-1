...
class FlinkDataSet[A: Meta: TypeInformation] private[api](@transient private val rep: DataSet[A]) extends DataBag[A] {

  override type TypeInfo[T] = TypeInformation[T]
  ...
  override def fold[B: Meta: TypeInfo](z: B)(s: A => B, u: (B, B) => B): B = {
  ...
  override def map[B: Meta: TypeInfo](f: (A) => B): DataBag[B] =
    rep.map(f)

  override def flatMap[B: Meta: TypeInfo](f: (A) => DataBag[B]): DataBag[B] =
  ...
  override def groupBy[K: Meta: TypeInfo](k: (A) => K): DataBag[Group[K, DataBag[A]]] = {
    ???
...
//  }
  ...
  implicit def wrap[A: Meta: TypeInformation](rep: DataSet[A]): FlinkDataSet[A] =
    new FlinkDataSet(rep)

  def apply[A: Meta: TypeInformation]()(implicit flink: ExecutionEnvironment): FlinkDataSet[A] =
    flink.fromElements[A]()

  def apply[A: Meta: TypeInformation](seq: Seq[A])(implicit flink: ExecutionEnvironment): FlinkDataSet[A] =
