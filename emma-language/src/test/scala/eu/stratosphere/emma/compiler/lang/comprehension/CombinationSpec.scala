...
class CombinationSpec extends BaseCompilerSpec {
  ...
  def getSingleApplPipeline(rule: Tree =?> Tree): Expr[Any] => Tree =
    compiler.pipeline(typeCheck = true)(
      Core.resolveNameClashes,
      Core.anf,
  ...
  "match single" - {
    "filter" in {
      ...
      val matchFilterOnce = getSingleApplPipeline(Combination.MatchFilter)
      ...
      matchFilterOnce(inp) shouldBe alphaEqTo(anf(exp))
    }

    "flatMap" in {

      val matchFlatMapOnce = getSingleApplPipeline(Combination.MatchFlatMap)
      ...
      matchFlatMapOnce(inp) shouldBe alphaEqTo(anf(exp))
    }

    "flatMap2" in {

      val matchFlatMap2Once = getSingleApplPipeline(Combination.MatchFlatMap2)
      ...
      matchFlatMap2Once(inp) shouldBe alphaEqTo(anf(exp))
    }

    "cross" in {

      val matchCrossOnce = getSingleApplPipeline(Combination.MatchCross)
      ...
      matchCrossOnce(inp) shouldBe alphaEqTo(anf(exp))
    }

    "cross (complicated)" in {
      ...
      val exp = reify {
        comprehension[String, DataBag] {
          ...
          val c = generator[(User, Int), DataBag] {
            ir.cross(u, DataBag(Seq(1,2,3)) plus DataBag(Seq(4,5,6)))
          }
          val z = generator[Long, DataBag] {
    ...
    "equiJoin" in {
      ...
      val matchEquiJoinOnce = getSingleApplPipeline(Combination.MatchEquiJoin)
      ...
      val exp = reify {
        comprehension[(User, User), DataBag] {
          val j = generator[(User, User), DataBag] {
            val k1 = (x: User) => x.id
            val k2 = (y: User) => y.id
            ir.equiJoin(k1, k2)(users$1, users$1)
          }
          head {
            (j._1, j._2)
          }
      ...
      matchEquiJoinOnce(inp) shouldBe alphaEqTo(anf(exp))
    }
    ...
    "equiJoin not applicable" in {
      ...
      matchEquiJoinOnce(inp) shouldBe alphaEqTo(anf(inp))
    }

    "equiJoin (complicated)" in {
    ...
    "residual" in {
      ...
      val matchResidualOnce: Expr[Any] => Tree =
        compiler.pipeline(typeCheck = true)(
          Core.resolveNameClashes,
          Core.anf,
          tree => api.TopDown.transform(Combination.MatchResidual)(tree).tree,
          Core.flatten
        ).compose(_.tree)
      ...
      matchResidualOnce(inp) shouldBe alphaEqTo(anf(exp))
      combine(inp) shouldBe alphaEqTo(anf(exp))
    }
  }
  ...
  "combine" - {
    "two crosses" in {
      ...
      val exp = reify {
        ir.cross(ir.cross(users minus users$1, users$1 plus users), DataBag(Seq(1,2,3)))
    ...
    "filter after cross" in {
      ...
      val exp = reify {
        ir.cross(users, users).withFilter(a => a._1.id != a._2.id).map(c => (c._1, c._2))
      }
  ...
  "combine nested" - {
    "in head" in {
      ...
      Core.flatten(combine(inp)) shouldBe alphaEqTo(anf(exp))
    }
  }
