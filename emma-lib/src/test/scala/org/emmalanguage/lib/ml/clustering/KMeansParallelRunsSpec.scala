...
class KMeansParallelRunsSpec extends lib.BaseLibSpec {
  ...
  "kMeansParallelRuns" should "cluster points around the corners of a hypercube" in {
  ...
  def clusters(solution: Set[kMeansParallelRuns.Solution[Long]]): Iterable[Set[Long]] =
    solution.groupBy(_.label.id).mapValues(_.map(_.id).toSet).values

  def run(k: Int, input: String): Set[kMeansParallelRuns.Solution[Long]] = {
    // read the input
    ...
    // do the clustering
    val result = kMeansParallelRuns(2, k, runs, iterations)(points)
    // return the solution as a local set
    result.collect().toSet[kMeansParallelRuns.Solution[Long]]
  }
}
